...
const topSongsController = {
    ...
    all (req, res, next) {
        ...
        var dataQuery = baseQuery.clone()
            .select('*')
            .from('songs')
            .where('pub_date', '<', moment().subtract(30, 'days').format('YYYY-MM-DD'))
            .orderBy('songs.streams_count', 'desc')
            .orderBy('songs.downloads_count', 'desc')
            .orderBy('songs.likes_count', 'desc')
...
module.exports = topSongsController;
